...
const scopeRows = [
  ...
  ['操作日志', '查询与详情', '支持时间范围、操作者、功能模块、操作类型筛选；列表按操作时间倒序分页，点击整行打开详情抽屉，展示操作内容、操作结果及必要的脱敏信息。'],
...
const queryRows = [
  ...
  ['列表与详情', '列表默认按操作时间倒序并支持分页；变更时间、来源或任一筛选条件后回到第 1 页，点击整行打开详情抽屉。'],
...
const exceptionRows = [
  ['场景', '反馈与恢复'],
  ['查询失败', '查询接口失败时页面应展示失败状态和“重新加载”；原型通过可控评审场景模拟失败、重新加载和恢复。服务端应提供可读错误信息，页面不展示旧数据或虚构内容。'],
  ['无数据', '按当前 Tab 和筛选条件展示对应空状态。'],
  ['详情缺失', '显示“详情暂不可用”，不影响继续查看列表。'],
  ['导出失败', '展示可读失败原因并允许重试；原型可切换下次导出的成功或失败结果，导出中防止重复提交，并按实际结果生成门户操作留痕。'],
...
const acceptanceRows = [
  ...
  ['分页与详情交互', '列表支持跨页查看；切换 Tab、时间或筛选条件后回到第 1 页，点击任意日志整行打开对应详情抽屉。'],
  ...
  ['查询失败反馈', '查询接口失败时页面展示失败状态和“重新加载”；通过原型可控 mock 验收失败、加载中与恢复。'],
